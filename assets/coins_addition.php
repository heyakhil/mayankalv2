					...
					if (mysqli_num_rows($result) > 0) {
					    ...
					    while($row = mysqli_fetch_assoc($result)) {
							...
							if (mysqli_query($conn, $sql)) {
							    header("location:https://www.mayankal.ml/dashboard/earning.php?msg=Your 30 coins is added in your account");
							} else {
			      ...
			      $sql = "SELECT * FROM coins_earn WHERE `uid`='$uid'";
					...
					if (mysqli_num_rows($result) > 0) {
					    ...
					    while($row = mysqli_fetch_assoc($result)) {
							...
							if (mysqli_query($conn, $sql)) {
								header("location:https://www.mayankal.ml/dashboard/earning.php?msg=Your 30 coins is Added");
							} else {
							    echo "Error updating record: " . mysqli_error($conn);
							}
					    }
