...
if (isset($_GET['send']) AND isset($_GET['receive'])) {
	...
	$send = $_GET['send'];
	if ($_GET['send']==$_GET['receive']) {
		?>
		<script type="text/javascript">
			window.open("../single-freelancer-profile.php?uid=<?php echo $receive; ?>", "_self");
			alert("Your Can Not Give Review To Yourself");
		</script>
		<?php
	}else{
		...
		if (mysqli_query($conn, $sql)) {
			...
			if (mysqli_num_rows($result) > 0) {
			    ...
			    while($row = mysqli_fetch_assoc($result)) {
			       ...
			       $sql = "UPDATE user_info SET rating='$data_rate' WHERE `uid`='$receive'";
					if (mysqli_query($conn, $sql)) {
					    ?>
				    <script type="text/javascript">
				    	window.open("../user-profile.php?uid=<?php echo $receive; ?>", "_self");
